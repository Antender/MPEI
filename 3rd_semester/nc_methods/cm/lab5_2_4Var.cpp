// lab5.2(4Var).cpp: определяет точку входа для консольного приложения.
//
...
void F1(double mas[][nmax], double z[], double x[], int N)
{
	...
	for (int i = 0; i <= N; i++)
		for (int j = 0; j <= N; j++)
		{
			...
			mas[N][N] = 1;
			if ((i != 0) & (i != N))
				mas[i][i] = 2+h*h*exp(-x[i])-(3/2)*h*h*sqrt(z[i]); //(2+h*h*(2+x[i]))-h*h*(1/(2*sqrt(1+z[i])));
			if  (i != N-1)
				mas[i+1][i] = -1;
			mas[i+1][i+2] = -1;
		}
...
void F(double mas[], double z[], double x[], int N, double grA, double grB)
{
	...
	for (int i = 1; i < N; i++)
		{
			mas[i] = -(-z[i-1]+2*z[i]-z[i+1]+exp(-x[i])*z[i]*h*h-h*h*z[i]*(sqrt(z[i])));
		}
	mas[N] = -z[N]+grB;
}
...
void diagElem(double mas[], double z[], double x[], int N)
{
	...
	for (int i = 0; i <= N; i++)
	{
		mas[0] = 1;
		if ((i != 0) & (i != N))
			mas[i] = 2+h*h*exp(-x[i])-(3/2)*h*h*sqrt(z[i]); //(2+h*h*(2+x[i]))-h*h*(1/(2*sqrt(1+z[i])));
		mas[N] = 1;
	}
}
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0; i <= N; i++)
	{
		cout << temp[i] << ' '; // вообще тут должно быть z[i], а temp[i] это предыдущее вычисление
	}
	cout << '\n';
